...
import display.addnewframe.admin.PenaltyPolicyAddFrame;
import display.editframe.admin.PenaltyPolicyEditFrame;
import display.event.TableActionEvent;
import display.viewframe.admin.PenaltyPolicyViewFrame;
...
import saveinformation.SavePolicyInfo;
...
public class FormPolicy extends javax.swing.JPanel {
    ...
    public FormPolicy() {
        initComponents();
        //  add row table
        List<PenaltiesPolicy> list = policyDao.findAllPolicies();
        for (PenaltiesPolicy b : list) {
            Object[] row = {
                b.getId(),
                b.getName(),
                Format.getVNCurrency(b.getPrice())
            };
            tablePolicy.addRow(row);
        }
...
//                System.out.println("Edit row: " + row);
                ...
                PenaltiesPolicy policy = policyDao.findPolicyById(id);
                SavePolicyInfo.saveInfo(policy);
                PenaltyPolicyEditFrame editFrame = new PenaltyPolicyEditFrame();
                editFrame.setVisible(true);
            }

            @Override
            public void onDelete(int row) {
                if (tablePolicy.isEditing()) {
                    tablePolicy.getCellEditor().stopCellEditing();
                }
                DefaultTableModel model = (DefaultTableModel) tablePolicy.getModel();
                int id = (int) model.getValueAt(row, 0);
                try {
                    boolean isDelete = policyDao.delete(id);
                    if (isDelete) {
                        model.removeRow(row);
                    } else {
            ...
            public void onView(int row) {
                ...
                PenaltiesPolicy policy = policyDao.findPolicyById(id);
                SavePolicyInfo.saveInfo(policy);
                PenaltyPolicyViewFrame viewFrame = new PenaltyPolicyViewFrame();
                viewFrame.setVisible(true);
            }
        };
        TableColumn col = tablePolicy.getColumnModel().getColumn(3);
...
//    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Penalties Policies");
        ...
        tablePolicy.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1), "", javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("SansSerif", 0, 12), java.awt.Color.lightGray)); // NOI18N
        tablePolicy.setForeground(new java.awt.Color(102, 102, 102));
        ...
        tablePolicy.setFont(new java.awt.Font("SansSerif", 0, 12)); // NOI18N
        tablePolicy.getTableHeader().setReorderingAllowed(false);
        spTable.setViewportView(tablePolicy);
    ...
    private void btnExpCSVActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExpCSVActionPerformed
        ...
        exportPenaltyPolicies(FileChoose.fileChooser());
    }//GEN-LAST:event_btnExpCSVActionPerformed
    private void exportPenaltyPolicies(String filePath) {
        List<PenaltiesPolicy> policyList = policyDao.findAllPolicies();
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath + ".csv"))) {
            ...
            writer.newLine();
            for (PenaltiesPolicy policy : policyList) {
                writer.write(policy.getId() + "");
                writer.write(",");
                writer.write(policy.getName());
                writer.write(",");
                writer.write(policy.getPrice().toString());
                writer.write(",");
                writer.write(policy.getCreated() + "");
                writer.newLine();
            }
    ...
    private void btnAddNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddNewActionPerformed
        // TODO add your handling code here:
        PenaltyPolicyAddFrame addFrame = new PenaltyPolicyAddFrame();
    ...
    private swing.Table tablePolicy;

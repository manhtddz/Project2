...
import display.addnewframe.admin.OptionAddFrame;
import display.editframe.admin.OptionEditFrame;
import display.event.TableActionEvent;
import display.viewframe.admin.OptionViewFrame;
...
//                System.out.println("Edit row: " + row);
                TableModel model = tableOption.getModel();
                int id = (int) model.getValueAt(row, 0);
                Options option = optionDAO.findOptionById(id);
                SaveOptionInfo.saveInfo(option);
            ...
            public void onDelete(int row) {
                if (tableOption.isEditing()) {
                    tableOption.getCellEditor().stopCellEditing();
                }
                DefaultTableModel model = (DefaultTableModel) tableOption.getModel();
                int id = (int) model.getValueAt(row, 0);
                try {
                    boolean isDelete = optionDAO.delete(id);
                    if (isDelete) {
                        model.removeRow(row);
                    } else {
            ...
            public void onView(int row) {
                ...
                Options option = optionDAO.findOptionById(id);
                SaveOptionInfo.saveInfo(option);
                OptionViewFrame viewFrame = new OptionViewFrame();
                viewFrame.setVisible(true);
            }
        };
        TableColumn col = tableOption.getColumnModel().getColumn(4);
...
//    }
    ...
    private void btnAddNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddNewActionPerformed
        // TODO add your handling code here:
        OptionAddFrame addFrame = new OptionAddFrame();
